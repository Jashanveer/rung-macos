...
struct HabitCard: View {
    ...
    /// macOS phrasing differs from iOS: HealthKit on Mac is read-only,
    /// so verification actually runs on the user's iPhone — the copy
    /// reflects that to avoid the "Waiting forever" footgun.
    ...
    var body: some View {
        ...
        .contextMenu {
            ...
            if isHabitEntry {
                ...
                // item so users can't bypass HealthKit/Screen Time evidence
                // from a device that can't actually run the verifier.
...
/// Health for HealthKit-backed habits, Screen Time for the social-media
/// limiter. Sized small (caption) so it fits inline with the title.
